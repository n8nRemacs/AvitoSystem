...
### §4.2.6 ★★ Section-level severity inheritance / inline edit at every level (запрос юзера 2026-05-14 перед сном)

Юзер: «возможность правки применимых дефектов в каждом разделе с прокидыванием вниз по разделам, а то сейчас только в корне можно задать и дальше оно не редактируемо.»

Два возможных прочтения — **обязательно уточнить у юзера на сессии 2026-05-15 перед имплементацией:**

**Интерпретация A — feature-tree inheritance (вероятно):** Дополнительное измерение наследования. Сейчас resolver проходит вверх ТОЛЬКО по device tree (Phone → Apple → iPhone 13). Bindings определены per (device_node, feature_node — конкретный defect). Юзер хочет добавить наследование и через feature tree: настроить severity на разделе «Дисплей» → все 3 defect-child под ним получат тот же severity, если у них нет своего binding. Это нужно для DRY когда дефекты в разделе должны иметь одинаковую severity по умолчанию.

**Интерпретация B — упростить device-tree edit:** Убрать промежуточный клик «Задать здесь». Сейчас на iPhone 13 видишь 6 bindings как inherited (read-only), нужно кликнуть Override → копируется на iPhone 13 → становится editable. Хочется чтобы dropdowns были editable сразу. Минус: implicit overriding может удивлять (юзер думал что меняет родителя, а на самом деле создал override на child'е).

**Архитектурно:**
- A требует extension resolver'а — добавить walk-up feature-tree в дополнение к walk-up device-tree. Двумерная inheritance matrix. Spec нужен.
- B — UI-only: убрать "Задать здесь" кнопку + при изменении dropdown сделать auto-POST binding на текущий device. Простой, но меняет UX-pattern (implicit override).

**Гипотеза:** скорее всего юзер имел в виду A (явно сказал «по разделам» = sections feature tree). Но quick fix B тоже стоит обсудить.

**Это входит в Project B scope** (Признаки UI читает из catalog). Brainstorm на сессии 2026-05-15.
