...
| `avito-monitor/app/data/dialog_topics.yaml` | **Source of truth** для baseline тем seller-dialog опроса. Сейчас 11 тем для iPhone 12 Pro Max. Расширять по мере добавления новых моделей в профилях. Изменения мержатся в `dialog_topics` таблицу через ON CONFLICT UPDATE (повтор миграции 0014 или manual upsert). | Да |
