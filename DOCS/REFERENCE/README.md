...
**Обновлено:** 2026-05-13 (вечер) — **Phase 2.1 hotfixes + Project A defect-catalog зашиплены**:
- **Phase 2.1 hotfixes на main**:
  - **F1 prompt fix** — `extract_price_signal.md` переписан с STRICT JSON SHAPE block + canonical/wrong examples + rejection-warning. LLM возвращал `repaired_components` как bare array (71% drift) вместо `{"items":[...]}`. Template hotfix `4af53c4` остаётся defence-in-depth.
  - **F5 test fix** — `tests/seller_dialog/test_view.py::test_phase_a_stages_aliases_phase_b` rename + assertion update под Phase B alias (3 stages).
  - **Parser-tune recall** (commit `c562971`) — все 6 section-prompts получили expanded «ok» definition (positive confirm OR **negation-of-defect** OR общее положительное состояние) + explicit euphemism lists под «defect» + concrete «Примеры распознавания» блок. После tune: на iPhone 12 PM red 5→14 (+9 confirmed defects), sensors.sim ok 10→34, operability.no_boot ok 0→8.
  - **Bucket relax** (commit `7e44d3b`) — `compute_bucket` теперь: explicit `state='unknown'` НЕ блокирует green; только missing-feature (None default) → grey (preserves «never scanned» safety). Структурно: с 20 active rules вероятность всех state='ok' = практически 0; relax даёт «no confirmed defect on any rule + all rules scanned → green» как реалистичный сигнал. Backfill_features.py обновлён persist'ить `profile_listings.bucket` для smoke-runs.
- **Project A — defect-catalog admin tool** (ветка `feat/defect-catalog`, 30 commits, **deployed на prod 2026-05-13**, НЕ merged в main до manual UI smoke):
  - **Архитектура**: новая admin-страница `/defects` с двумя ортогональными иерархиями + bindings. Чисто config-only — НЕ затрагивает текущий compute_bucket / ProfileFeatureRule / kanban (это будет Project B).
  - **Schema** (`0017_defect_catalog`): 3 новые таблицы.
    - `feature_nodes`: tree узлов и defect-leaves (`kind ∈ {'node','defect'}`) с self-FK parent_id CASCADE + UNIQUE(parent_id, slug) + optional `prompt_hint` поле для будущего Project C.
    - `device_nodes`: tree устройств (Phone → Apple → iPhone 12 Pro Max, free depth) с self-FK parent_id CASCADE + UNIQUE(parent_id, slug).
    - `device_feature_bindings`: связь device→feature с 2-dim severity (`defect_action ∈ {'block','info'}`, `unknown_action ∈ {'ask','skip'}`) + `disabled` boolean (N/A flag) + UNIQUE(device_node_id, feature_node_id). Bindings только на kind='defect' (enforced в repository layer).
  - **Resolver** (`app/services/defect_catalog/resolver.py`): walks device tree up от target, для каждого feature берёт nearest-ancestor binding (CSS-style каскад), drop disabled. Returns `ResolvedBinding(feature_path, defect_action, unknown_action, inherited_from)`. Поддерживает override (child binding wins) и N/A (disabled=True пропускает binding даже если есть у предка).
  - **Repository** (`app/services/defect_catalog/repository.py`): raw SQL via `text()` с dialect-aware `_now_expr(session)` helper (SQLite vs Postgres). Cycle detection для parent_id changes. Slug validation regex `^[a-z][a-z0-9_]*$`. **Backlog** (CQ-flagged): миграция на ORM, per-call commit → caller commits, N+1 queries → CTE.
  - **Admin UI**: `/defects` (redirect → /defects/devices), 2 таба (Устройства / Признаки). Split-pane на Устройства tab: device tree слева, applicable defects справа с inherited-vs-set-here разным UX (inherited = read-only + Override button, set-here = editable dropdowns + Удалить). Catalog tab = simple tree editor с `<details>` collapse. HTMX inline edits для severity (PATCH /bindings/{id}). FastAPI + Jinja templates в `app/web/templates/defects/`.
  - **Routes** (8 GET + POST + PATCH + DELETE для bindings + POST/PATCH/DELETE для nodes): `/defects/devices/tree`, `/defects/devices/{id}`, `/defects/catalog/tree`, `/defects/bindings` (POST/PATCH/DELETE), `/defects/devices` (POST), `/defects/catalog` (POST), `/defects/devices/{id}` (DELETE + PATCH on /edit), `/defects/catalog/{id}` (DELETE + PATCH on /edit). Sidebar nav entry «🛠 Дефекты».
  - **Seed** (`scripts/seed_defect_catalog.py`): идемпотентный, stable UUIDs через `uuid.uuid5(NS, "feature:case/back_broken")` etc. 8 features (Корпус×3 + Дисплей×3) + 3 devices (Phone → Apple → iPhone 12 PM) + 6 bindings (все на Phone-level → наследуются по device tree).
  - **Реализация** через subagent-driven-development (Sonnet для implementers, Opus для reviewers + parent). 34 tasks, 30 commits, 33 unit/integration тестов pass, 0 регрессий в существующих. Spec `DOCS/superpowers/specs/2026-05-13-defect-catalog-design.md`, план `DOCS/superpowers/plans/2026-05-13-defect-catalog-impl.md`. Project B (Признаки UI читает из catalog) и Project C (auto-gen LLM prompts из catalog `prompt_hint`) — отдельные spec+plan когда юзер скажет.
  - **Critical fixes по review**: (1) `datetime('now')` в update_*_node — SQLite-only, фейлит на Postgres → dialect-aware `_now_expr` helper. (2) `app/web/routers/` directory shadow'ит существующий `app/web/routers.py` (ImportError на web_router) → новый роутер переехал в `app/web/defects.py` как sibling.
...
| Defect feature pipeline | **Phase 2.1 unified + post-deploy hotfixes shipped 2026-05-13.** 31 фича в `dialog_topics.yaml` (26 defect + 2 price_signal + 3 info_api). `listing_features` с `kind` discriminator + `value` JSONB. `analyze_listing_features` orchestrates 3 things in one pass: section-parser defect features + batched LLM price_signal + pure-Python info_api. **Section-parser prompts получили negation-as-ok recall fix 2026-05-13** (red on iPhone 12 PM went 5→14). `compute_bucket` relaxed: `unknown` ≈ ok (only missing-feature → grey for «never scanned» safety). |
| **Defect catalog admin** | **Project A shipped 2026-05-13** (alembic 0017, ветка `feat/defect-catalog`, НЕ merged). Новая admin-страница `/defects` с двумя ортогональными иерархиями (feature catalog + device tree) + bindings с 2-dim severity (defect_action × unknown_action). Config-only, не интегрирован в pipeline. Seed: Phone → Apple → iPhone 12 Pro Max + Корпус/Дисплей с 6 defects. Spec `DOCS/superpowers/specs/2026-05-13-defect-catalog-design.md`. Project B (Признаки UI читает из catalog) + Project C (auto-gen prompts) предстоят. |
...
| `DOCS/superpowers/specs/2026-05-12-unified-criteria-design.md` | **Spec Phase 2.0+2.1** — unified `listing_features` schema (3 kinds: defect / price_signal / info_api), V2 rip, 31-feature taxonomy, info-фичи на карточке, profile-edit form tabs restructure. 12 Q-решений из брейнсторма. | Нет (исторический) |
| `DOCS/superpowers/plans/2026-05-13-unified-criteria-phase-2.1.md` | План Phase 2.1 (executed) — 14 tasks. | Нет (исторический) |
| `DOCS/superpowers/specs/2026-05-13-defect-catalog-design.md` | **Spec Project A** — global defect catalog с двумя ортогональными иерархиями (feature catalog + device tree) + bindings таблица с 2-dim severity. Admin-only `/defects` page, чисто config-storage без интеграции в pipeline (Project B позже). 7 Q-решений из брейнсторма (глобальный catalog, свободная глубина дерева, 2-dim severity matrix, bindings только на defect-leaves, inheritance bottom-up). | Нет (исторический) |
| `DOCS/superpowers/plans/2026-05-13-defect-catalog-impl.md` | План Project A (executed) — 34 задачи subagent-driven, 30 commits на ветке `feat/defect-catalog`. 33 новых unit/integration теста. Spec coverage check в конце. | Нет (исторический) |
| `avito-monitor/app/services/defect_catalog/repository.py` | **Project A repository.** Raw SQL CRUD для feature_nodes / device_nodes / device_feature_bindings + slug validator + cycle detection + dialect-aware `_now_expr(session)` helper. Backlog: миграция на ORM, per-call commit → caller commits. | Да |
| `avito-monitor/app/services/defect_catalog/resolver.py` | **Project A resolver.** `resolve_applicable_defects(session, target_device_id)` walks device tree up, для каждого feature берёт nearest-ancestor binding, drop disabled. Returns sorted list of `ResolvedBinding` dataclasses. Backlog: N+1 → single CTE query при масштабе. | Да |
| `avito-monitor/app/web/defects.py` | **Project A router.** Все 12+ endpoints `/defects/*`. Sibling к `app/web/routers.py` (НЕ `app/web/routers/defects.py` — name collision с существующим routers.py файлом). HTMX-driven inline edits, error fragments на ValueError. | Да |
| `avito-monitor/app/web/templates/defects/` | **Project A templates.** `_layout.html` (extends global `_layout.html`, tabs), `devices.html`/`catalog.html` (tab stubs с HTMX `hx-get` для tree partials), `_partials/device_tree.html`, `_partials/feature_tree.html`, `_partials/device_detail.html`, `_partials/binding_row.html` (dual-mode: inherited read-only с Override button vs set-here с editable dropdowns), `_partials/bindings_list_fragment.html`. | Да |
| `avito-monitor/scripts/seed_defect_catalog.py` | **Project A seed.** Идемпотентный (`INSERT ON CONFLICT DO NOTHING` для Postgres / `INSERT OR IGNORE` для SQLite тестов), stable UUIDs через `uuid.uuid5(NS, "feature:slug/path")`. Текущий seed: 8 features (Корпус×3 + Дисплей×3) + 3 devices + 6 bindings на Phone-level. Расширять руками или через admin UI. | Да |
