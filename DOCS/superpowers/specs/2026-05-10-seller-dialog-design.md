...
**Дата:** 2026-05-10 (revision 3 — добавлен этап «Настройка опроса» и per-model severity в Phase 2 backlog)
...
| D13 | SLA-модель | **Тихий auto-reject** на silence-этапах (1 Контакт, 3 Опрос, 5 Цена изменена). Глобальный default 24ч. Никаких эскалаций к оператору, никаких TG-пингов на silence — оператор не должен тратить внимание на мёртвые лоты |
| D14 | TG-пинги оператору | Четыре триггера, все «нужно действие оператора»: продавец откликнулся (→ настрой темы), recap готов (→ торг), цена изменилась в лоте (→ выкупать), товар получен (→ закрыть сделку) |
...
```
  ...
  Из любого этапа → [Отказы]:
   • Silence timeout (на этапах 1/3/5) — auto-reject silently
...
| 7 | Отправка совершена | бот auto-wait | Тихо | items/{id} shipment marker / messenger keywords («отправил», tracking number) | Detect shipment → SUGGEST оператору перевод в этап 8 |
...
**Только** на этапах 1 / 3 / 5 (где мы реально **ждём действия продавца**). На operator-driven этапах (2 / 4 / 6 / 9) и Avito-delivery wait этапах (7 / 8) SLA не применяется.
...
- 1. Контакт: 24ч
- 3. Опрос: 24ч
- 5. Цена изменена: 48ч (продавцу нужно зайти и поменять ценник в листинге — даём чуть больше)
...
|---|---|---|
| Контакт → Настройка опроса | «Лот N: продавец откликнулся. Настрой темы для опроса» | [Открыть карточку] |
| Опрос → Согласование цены | «Лот N: продавец подтвердил все темы. Готов к торгу» | [Открыть карточку] |
| Цена изменена → Лот выкуплен | «Лот N: продавец обновил цену в лоте. Выкупай» | [Открыть карточку] |
| Товар получен → Сделка закрыта | «Лот N: товар прибыл. Проверь и закрой сделку» | [Открыть карточку] |
...
- **Точные silence-timeout** числа (сейчас default 24ч / 24ч / 48ч на этапах 1/3/5) — финальные значения после первого soak-period.
...
- Action-required TG-пинги (4 триггера) — 1ч
