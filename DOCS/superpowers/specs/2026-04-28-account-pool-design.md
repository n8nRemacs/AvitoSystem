...
| D15 | Multi-phone support | Поддерживается архитектурно. `avito_accounts.phone_serial` TEXT (ADB serial). `device_switcher` расширен до `switch_to(phone_serial, android_user_id)`. ADB-lock per-phone (не глобальный). Сейчас все 2 аккаунта на одном телефоне (`110139ce`), но добавление 2-го OnePlus — конфигурация без code-changes. |

## 4. Architecture overview

```
┌──────────────────────────────────────────────────────────────┐
│ avito-xapi (LXC, USB passthrough к N OnePlus телефонам)       │
│  ┌─────────────────────┐    ┌────────────────────────────┐   │
...
CREATE TABLE avito_accounts (
    ...
    last_device_id         TEXT,                             -- Avito-app device_id (для refresh-cmd)
    phone_serial           TEXT NOT NULL DEFAULT '',         -- ADB serial физического телефона
...
Для добавления 2-го физического OnePlus в будущем: подключить USB → проверить `adb devices` показывает оба → задать `phone_serial` нового аккаунта на новый serial. Никаких code-changes.
    ...
    if acc.state in ('active', 'needs_refresh') and \
       (acc.expires_at < now() + timedelta(minutes=3) or acc.state == 'needs_refresh'):
        ...
        if not await device_switcher.health(acc.phone_serial):
            await tg_alert(f"ADB-канал к телефону {acc.phone_serial} отвалился")
            return
...
Поддерживает N физических телефонов одновременно. Каждый физический device идентифицируется по ADB serial (`adb devices` показывает; не путать с Avito-app `device_id`).

```python
class DeviceSwitcher:
    def __init__(self):
        self._locks: dict[str, asyncio.Lock] = {}  # per-phone
    ...
    async def switch_to(self, phone_serial: str, target: int) -> None:
        async with self._lock_for(phone_serial):
            ...
            # ждать current_user(phone_serial) == target до 5 сек
    ...
    async def list_devices(self) -> list[str]:
        # adb devices → список serial'ов в state=device
        # для bootstrap-проверки и админ-эндпойнта
```

**Параллелизм:** switch на разных phone_serial'ах не блокирует друг друга (разные locks). Это критично если у нас 2 OnePlus и обоим одновременно нужен refresh — оба процесса идут параллельно.
...
LXC config (Proxmox): cgroup devices.allow для всех USB-узлов + bind-mount `/dev/bus/usb` (целиком, не один node). Это позволяет hot-plug новых телефонов без перезапуска контейнера.
...
| ADB не отвечает на phone_serial | TG-alert «ADB-канал к {serial} отвалился», refresh откладывается; аккаунты на других телефонах работают |
...
- `DeviceSwitcher`: `switch_to` no-op когда уже там, retry, asyncio.Lock сериализация per-phone. **Параллельные `switch_to` на разных phone_serial'ах не блокируют друг друга** (отдельный тест: lock_for(A) и lock_for(B) — разные lock'и). Mock `subprocess.run` с argv анализом (проверяем `-s {serial}` присутствует).
...
5. LXC config: USB passthrough всех bus/usb узлов + проверить `adb devices` из контейнера показывает все подключённые телефоны в state `device`.
...
- APK auto-detect `android_user_id` и `phone_serial` через системные API → POST /sessions автоматически проставляет (избавляет от ручного UPDATE при добавлении нового аккаунта).
